...
$checkout_year = '2015';
	...
	class PDF extends FPDF
	{
			...
			function Header(){
				...
				$this->Ln(20);
				$this->Cell(5);
				$this->Cell(0,10,'CHECK-OUT LIST FOR THE MONTH OF '. strtoupper($this->months[$this->month-0]).' '.$this->year,10,0,'C');
				// Line break
				$this->Ln(20);
				// Header
	...
	if(is_array($list)){
	    ...
	    foreach($list as $row){
			extract($row);
			$db_data[] = array(++$x, $cp_no ,$IndexNo, $firstName, $lastName, $sex, $eom, $cntr_code,  $MHQDate);
	...
	$pdf = new PDF();
	$pdf->month = $checkout_month;
	$pdf->year = $checkout_year;
	
    $pdf->table_header = array( '#', 'CPNo.', 'Index', 'First Name', 'Last Name', 'Sex', 'EOM', 'CNTRY', 'MHQ Date'); 
	$pdf->AliasNbPages();
	$pdf->AddPage();
	$pdf->FancyTable($db_data);
	$pdf->Output();
