	...
	function bank_details(display_id,mode){
		...
		var xmlhttp;
		var args;
		var ipo_cp = document.getElementById('IPO_CP' + display_id).value;
	...
	//	if(isAlphaNumeric(branch_address)  == false)
		...
		url_args = "&n=" +bank_name + "&b="  + branch_name + "&a=" + branch_address + 
					"&r=" + routing + "&i=" + iban + "&s=" + swift_code + "&c=" + account + "&m=" + mode + "&cp=" +ipo_cp;
		...
		xmlhttp.onreadystatechange=function(){					
			...
			if (xmlhttp.readyState==4 && xmlhttp.status==200){				
			    ...
			    if(response == 1){ 
			        document.getElementById("mode"+display_id).value = 'e';
			        document.getElementById("msg"+display_id).innerHTML= "Saved";
			    }	
			    else{
			        document.getElementById("msg"+display_id).innerHTML= response;
			    }
